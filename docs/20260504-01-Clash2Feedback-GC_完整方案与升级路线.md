...
| Level 3 | 几何引导采样 | 否 | 是 | 是 | 只有实现 clash penalty / hot region guidance 并改采样过程后, 才能声称旧碰撞热区进入生成过程 |
...
当前短期不要把 learned critic / learned adapter / ranker 写成阶段 3 或阶段 4.0 的必做项. 近期顺序应为:
...
| 数据 | 统一保存位置 | 用途 |
|---|---|---|
| 人工注入局部碰撞集 | `data/benchmarks/clashrepairbench_rg_artificial/v0_1/` | 有人工 target R-group 标签, 用于标签溯源、mask seed 和 controlled local repair |
| 生成器诱导失败集 | `data/benchmarks/model_induced/v0_1/` | 更接近真实生成失败，用于最终验证 |

阶段 2.5 external validity audit 不直接构建可训练 benchmark, 而是生成 reports, 用于判断真实 generated failures 的 taxonomy 与 phase2 artificial supported cases 的分布差距. 当前阶段 2.5 最终报告显示, frozen DiffSBDD de novo complete-ligand audit 中 `single_rgroup_clash` 仅 `1 / 200` unique candidates, 因此不能声称真实 de novo failures 主要是 R-group clash. 若后续要把 model-induced failures 纳入 benchmark, 应另建 `data/benchmarks/model_induced/v0_1/`, 不要回改 `phase2_v0_1`.

人工注入局部碰撞集是 controlled synthetic failed pose benchmark, 不是热力学稳定结合构象证明。第一版按如下 split 保存:

| split | 含义 |
|---|---|
| `supported_single_rgroup` | target R-group 单区域主导, clean local repair substrate |
...
- `supported_single_rgroup` 是经过 detector / attribution / target-dominance gates 过滤后的 attribution-aware clean local repair substrate.
- `target_score_ratio_valid` 来自 attribution-derived valid R-group scores.
- supported 主集上的 Top-1 / Top-3 只能作为 construction consistency check, 不作为 independent locator benchmark.
...
阶段 1 已在 51 个 clean pool 样本和 28 个 balanced subset 样本上完成 clean calibration. 默认 `delta = 0.4 Å` 在当前 clean calibration 中 severe false positive 为 0, verifier clean-vs-clean smoke 为 28 / 28 pass. 这些结果支持进入 controlled synthetic failed pose 构造, 但不代表已经验证生成器修复, independent locator accuracy, 或 full-receptor checked repair.

阶段 2 benchmark 构造时可以记录 predicted `dominant_region`, 但不能把它作为唯一保留条件. 同时也要承认 `target_score_ratio_valid` 是 attribution-derived gate, 会让 supported 主集带有循环验证风险. 因此阶段 3 应改为 label provenance audit, circularity risk audit 和 phase4 mask seed generation.

使用 frozen DiffSBDD 或其他 SBDD baseline 前, 需要先做 training-overlap / provenance audit. 项目自身 val/test split 不等于 DiffSBDD unseen split. 若 pocket 可能出现在 baseline training set 中, 该结果只能作为 same-source diagnostic audit, 不能作为严格 external validity evidence.

阶段 2.5 的 `single_rgroup_clash` 是 detector / attribution taxonomy, 不是 oracle Top-1 ground truth; 不能用它计算 locator accuracy.
...
纠错器是长期阶段 6 的学习型模块, 不属于当前阶段 3 或阶段 4.0 的执行范围. 长期设计不只是二分类器，建议包含：
...
学习型反馈适配器属于阶段 7. 当前阶段 4.0 / 4.1 先使用规则适配器和 fixed backend controls; 学习型版本等待阶段 4.1 有稳定 repair outcomes 后再推进. 长期反馈适配器负责：
...
\[
\boxed{
\text{阶段 3 provenance / mask seed}
+
\text{阶段 4.0 backend feasibility}
+
\text{阶段 4.1 Random / Predicted / Oracle repair loop}
+
\text{可靠验证器}
}
\]
...
阶段 4 不应直接跳到大规模完整 repair loop. 正确顺序是:
...
阶段 4 的 predicted mask 是 operational mask policy, 不是 ground truth. Verifier 才是修复成功裁判, 成功标准应包括 old clash resolved, no new severe clash, ligand validity, scaffold RMSD, non-mask RMSD, anchor consistency 和 fixed-region preservation.

DiffDec / DiffSBDD plain backend 只能表述为 local constrained resampling 或 candidate inpainting backend. 在冻结原版生成后端设定下, `H_clash` 和 severity 多数情况下不会直接进入模型去噪过程; 只有实现 clash penalty / hot region guidance 并改 sampling / denoising loop 后, 才能声称旧碰撞热区参与生成过程.
...
| Predicted mask regeneration | attribution-derived operational mask policy |
| Structured predicted protocol | predicted mask + keep / anchor / severity + 规则适配器 |
| Learned critic + rule adapter | 后续阶段 6 学习型纠错器 + 规则适配器 |
| Learned critic + learned adapter | 后续阶段 6/7 学习型纠错器 + 学习型适配器 |
| Full method + ranker | 后续阶段 5/6/7 完整方法 |
| Oracle protocol | 人工 `target_rgroup` 对应区域和最优控制上限 |

### 17.2 主要指标

| 指标 | 含义 |
|---|---|
| R-group Top-1 / Top-3 construction consistency | 在 phase2 supported 主集上检查构造一致性, 不作为无偏定位指标 |
| Circularity risk level | detector / attribution / target-dominance gates 与评估方法的共享依赖风险 |
...
2. **提出结构化修复协议和评价协议**  
   明确 mask policy 改哪里、保留哪里、改多大，并用 verifier 判断旧错误是否被修掉。

3. **审计 attribution-derived operational mask policy 的下游价值**
   不把 phase2 supported 主集写成无偏 locator benchmark, 而是通过 Random / Predicted / Oracle repair loop 评估 predicted mask 的 downstream repair utility。
...
> Clash2Feedback-GC 先审计 controlled repair substrate 的标签来源和循环验证风险, 冻结 attribution-derived operational mask policy, 再通过同一 backend 和同一 candidate budget 下的 Random / Predicted / Oracle repair loop 判断 predicted mask 是否有 downstream repair utility。学习型纠错器、学习型适配器和排序器属于阶段 5/6/7, 只有在阶段 4.1 得到稳定结果后再作为增强模块推进。
...
3. 实现碰撞检测器和可靠验证器；
4. 构建人工局部碰撞数据；
5. 先做阶段 3 label provenance audit 和 phase4 mask seed；
6. 再做阶段 4.0 backend feasibility audit；
7. 随后做阶段 4.1 Random / Predicted / Oracle formal repair loop；
8. 只有阶段 4.1 稳定后, 再训练纠错器、排序器和轻量适配器。
