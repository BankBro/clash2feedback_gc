...
阶段 2.5 external validity audit 不直接构建可训练 benchmark, 而是生成 reports, 用于判断真实 generated failures 的 taxonomy 与 phase2 artificial supported cases 的分布差距. 若后续要把 model-induced failures 纳入 benchmark, 应另建 `data/benchmarks/model_induced/v0_1/`, 不要回改 `phase2_v0_1`.
...
阶段 1 已在 51 个 clean pool 样本和 28 个 balanced subset 样本上完成 clean calibration. 默认 `delta = 0.4 Å` 在当前 clean calibration 中 severe false positive 为 0, verifier clean-vs-clean smoke 为 28 / 28 pass. 这些结果支持进入 controlled synthetic failed pose 构造, 但不代表已经验证生成器修复, 规则 locator Top-1 / Top-3, 或 full-receptor checked repair.

阶段 2 benchmark 构造时可以记录 predicted `dominant_region`, 但不能把它作为唯一保留条件. 否则阶段 3 的 Top-1 / Top-3 评价会被构造过程泄漏和筛选偏差污染.
