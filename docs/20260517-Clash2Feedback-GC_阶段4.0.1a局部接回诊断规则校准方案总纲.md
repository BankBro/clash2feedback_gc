...
10. 在校准结果基础上新增 25 个样本的 visual QC 人工可视化抽查收尾补充，用于检查 reconnect 三分类和真实三维结构观察是否一致。
```
...
不把 visual QC 写成新生成实验或阶段 4.0.1a final report。
```
...
### 9.1 visual QC 人工可视化抽查收尾补充

阶段 4.0.1a visual QC 是本阶段的收尾补充，不是新生成实验。它只读取阶段 4.0.1a 已有 reconnect 校准表、阶段 4.0/4.0.1 既有候选和原始 clean/failed ligand，生成 25 个样本的可视化抽查资产、contact sheet、索引和人工/Codex review 模板。
...
不把 multi_attachment_out_of_scope 写成 ligand invalid；
不生成阶段 4.0.1a final report。
```

visual QC 的目标是检查 reconnect 三分类和真实三维结构观察是否一致。`multi_attachment_out_of_scope` 不等于 ligand invalid，只表示超出当前 single-anchor R-group repair 任务范围。
...
clean_positive: 3；
rule_positive: 3；
DiffSBDD invalid_reconnect 非 reliable: 6；
DiffSBDD multi_attachment_out_of_scope 非 reliable: 6；
DiffSBDD 原 reliable_repair_success=True 但 strict_single_anchor_shadow_reliable=False: 7。
```

每个候选生成三类 `3 x 4` contact sheet：
...
其中 `reconnect_clash_contact_sheet.png` 应尽量保持阶段 0 `clash_contact_sheet.png` 的总体风格：白色背景、soft lighting、candidate ligand orange stick、protein/pocket gray transparent、close-contact atoms sphere 和 BILD close-contact 标记。这个视图只回答 candidate 在 protein pocket 中是否仍有明显 close contact / clash / 空间异常，不混入大量 anchor/scaffold/generated fragment 标记。

`reconnect_anchor_topology_contact_sheet.png` 专门看 anchor reconnect / topology：generated fragment 是否接回指定 anchor，是否未接回 anchor，是否 floating fragment，是否 multi-attachment，extra attachment 是否真实。`reconnect_before_after_overlay_contact_sheet.png` 专门看 candidate 相对 failed/original 的局部变化，解释原 reliable 但 strict shadow fail 的原因。

Codex 或人工检视图片时必须遵守：

```text
如果图片检视基本支持自动三分类，则阶段 4.0.1a 可以进入正式关闭讨论；
如果图片检视发现大量不一致，应先修 reconnect / mapping 诊断，不得强行关闭为正结果；
如果 Codex 环境无法查看图片，不得编造视觉结论，只能生成图片、索引和人工 review 模板，并在临时汇报中说明阻塞。
```

visual QC 运行图片和中间可视化资产允许写入 `runs/phase4_0_1a_visual_qc/`，但 `runs/` 下 PNG、SDF、PDB、BILD、ChimeraX 脚本和日志默认不提交 Git。轻量 CSV/JSON/Markdown 索引写入 `reports/phase4_0_1a_local_reconnect_calibration/`，临时汇报写入 `tmp/20260517/phase4-0-1a-visual-qc-expt-report.md`。
...
14. 不提交 runs 下重资产候选 SDF、PNG、PDB、BILD、ChimeraX 脚本或日志；
...
reports/phase4_0_1a_local_reconnect_calibration/
tmp/20260517/
runs/phase4_0_1a_visual_qc/ （仅本地生成可视化运行资产，默认不提交）
```
