...
阶段 4 当前包含 `phase4_0_backend_feasibility` 后端可行性审计闭环: 支持 5 case preflight 和 40 case formal 小规模审计, 核查规则型固定拓扑局部构象修复, DiffSBDD CrossDocked full-atom conditional local completion, DiffSBDD full-ligand resampling 和 DiffDec single R-group scaffold decoration 是否能接入统一 verifier adapter. 该阶段不训练/微调模型, 不修改阶段 2/2.5/3 历史结果, 也不把 `H_clash` 写入 DiffSBDD/DiffDec 生成过程. DiffSBDD joint checkpoint 已进入 inventory, 但官方 inpaint 入口与 joint checkpoint 不兼容, 因此记录为 blocked. `scripts/phase4_0_closeout_diagnostics.py` 只读取既有阶段 4.0 结果并生成派生收尾诊断表和解释报告, 为 `phase4_0_final_experiment_report.md` 做准备, 不重跑任何 repair backend. 阶段 4.0.1 使用 `phase4_0_1_diffsbdd_conditional_repair` 独立修补 DiffSBDD conditional: 复用阶段 4.0 的 40 case, 固定 `center=pocket`, 运行 K=8/16/32 候选预算曲线, 输出 anchor/reconnect/fragment 诊断, 不覆盖阶段 4.0 历史结果, 不生成正式 final report. 阶段 4.0.1a 使用 `phase4_0_1a_local_reconnect_calibration` 对既有候选级表做 report-only / audit-only local reconnect 三分类校准, 并用 `phase4_0_1a_visual_qc` 抽查 25 个候选的 reconnect/clash/topology/overlay contact sheet 作为收尾证据. 阶段 4.0.1a 不重跑 DiffSBDD, 不重新生成候选, 不修改 reliable repair 10 项标准, 不生成 final report. 后续阶段 4.1 才讨论 Random / Predicted / Oracle formal repair loop.
