...
长期可复现入口见 `docs/external_baselines.md`, 其中记录 DiffSBDD / DiffDec source repo, pinned commit, checkpoint, 关键源码路径和输出口径. 外部源码非必要不修改; 必须修补 bug 或兼容性问题时, 补丁统一放在对应外部仓库的 `clash2feedback_gc` 分支, 且不得改变外部方法原有算法原理和语义. 单次运行的实际环境, GPU, smoke test 和命令记录写入对应阶段的 `reports/` 目录.
