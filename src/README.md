...
- `repair/`: 阶段 4.0 backend feasibility 审计, 阶段 4.0.1 DiffSBDD conditional repair 和阶段 4.0.1a local reconnect 校准, 包括 S2 preflight/formal case selection, phase2/base sample 输入合成, 规则型固定拓扑局部修复, DiffSBDD/DiffDec wrapper, topology-changing candidate verifier adapter, anchor/reconnect/fragment 诊断, 三分类 reconnect 审计和报告汇总.
